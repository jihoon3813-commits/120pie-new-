    ...
    <>
      ...
      {floatingSettings?.isActive && (
        <div className="fixed right-6 bottom-6 z-[90] flex flex-col items-end gap-3 font-bold text-xs select-none text-white">
          {floatingOpen && (
            <div className="flex flex-col items-end gap-2.5 mb-1.5 transition-all">
              {/* Instagram */}
          ...
          <button
            ...
            className={`w-12 h-12 rounded-full flex items-center justify-center transition-all cursor-pointer border-0 hover:scale-105 active:scale-95 ${
              floatingOpen
                ? "bg-[#735965] hover:bg-[#5d4752] rotate-45 text-white shadow-[0_6px_20px_rgba(115,89,101,0.35)]"
                : `${triggerBtnClosed} text-white`
            }`}
          >
            {floatingOpen ? (
