...
export function getInstagramThumbnailUrl(urlOrImg: string, fallbackLink?: string): string {
  ...
  // 1. 차단된 기존 legacy weserv.nl media/?size= URL은 엑박 방지를 위해 fallback 처리
  if (trimmed.includes("weserv.nl") && (trimmed.includes("media/?size=") || trimmed.includes("media/"))) {
    return INSTAGRAM_FALLBACK_IMAGE;
  }

  // 2. 이미 직렬 이미지 파일(Cloudinary, data:, cdninstagram 등)인 경우 그대로 반환
  if (
