  ...
  return (
    <div id={isPink ? "landing-v3" : "landing-v5"} className={`min-h-screen font-sans antialiased transition-colors duration-300 ${pageBg}`}>
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10 sm:py-16 space-y-24 sm:space-y-36">
        ...
        <section className={`rounded-3xl p-6 sm:p-12 ${cardBg} relative`}>
          <div className="flex items-center justify-between border-b border-neutral-200/20 pb-4 mb-8">
            <span className="text-xs font-black tracking-widest text-[#ffd500]">06 / INTERIOR STYLING</span>
            <span className="text-xs font-black text-slate-400">DESIGN LOOKBOOK</span>
          </div>

          <div className="space-y-10">
            <div className="text-center md:text-left space-y-3">
              <h2 className="text-2xl sm:text-3xl font-black">
                감성적이고 트렌디한 <span className={textHighlight}>내부 인테리어 연출</span>
              </h2>
              <p className={`text-sm sm:text-base leading-relaxed ${textDesc}`}>
                차콜 그레이와 우드, 옐로 네온 포인트를 조화시켜 B2B 디저트 카페만의 개성을 보여줍니다.
              </p>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center text-left">
              <div className={`rounded-xl overflow-hidden aspect-[16/10] relative group border ${isPink ? "bg-neutral-900 border-neutral-800" : "bg-white border-[#e6dfc3]"}`}>
                <img 
                  src="https://res.cloudinary.com/dx7l09wwu/image/upload/v1779945604/ChatGPT_Image_2026%EB%85%84_5%EC%9B%94_28%EC%9D%BC_%EC%98%A4%ED%9B%84_01_52_47_k3qg27.png" 
                  alt="120pie premium cafe front" 
                  className="w-full h-full object-cover opacity-85 group-hover:scale-102 transition-transform duration-350"
                />
                <div className="absolute inset-0 bg-gradient-to-t from-neutral-950/60 via-transparent to-transparent flex items-end p-4">
                  <span className="text-xs font-bold text-white/95">시그니처 차콜 그레이와 우드가 매칭된 프리미엄 매장 전경</span>
                </div>
              </div>
              <div className="space-y-4">
                <span className="text-xs font-bold text-[#ffd500] flex items-center gap-1.5">
                  <Sparkles size={16} /> INTERIOR KEY POINTS
                </span>
                <div className="space-y-3">
                  {[
                    { title: "조명 설계", desc: "갓 조명과 에디슨 웜톤 전구로 디저트 질감이 가장 돋보이는 분위기 구현", icon: <Sun size={16} className="text-amber-400" /> },
                    { title: "소재 선택", desc: "차콜색 모노타일 마감과 고품질 참나무 우드 상판 조합으로 클래식 감성 부여", icon: <Sliders size={16} className="text-amber-400" /> },
                    { title: "시그니처 간판", desc: "노랑/파랑 브랜드 컬러 스플래시가 반영된 입체 아크릴 사인보드로 외부 시인성 확보", icon: <Sparkles size={16} className="text-amber-400" /> }
                  ].map((pt, idx) => (
                    <div key={idx} className={`p-3 ${innerCardBgAccent} ${innerCardHover} rounded-xl text-left flex gap-3 items-start`}>
                      <div className="mt-1 p-1 rounded-lg">
                        {pt.icon}
                      </div>
                      <div>
                        <h5 className={`text-xs font-black ${textTitle}`}>{pt.title}</h5>
                        <p className={`text-[11px] ${textDesc} mt-1 leading-normal font-semibold`}>{pt.desc}</p>
                      </div>
                    </div>
                  ))}
                </div>
          ...
          <div className="flex justify-between items-center border-t border-neutral-200/20 pt-6 mt-8 text-[11px] sm:text-xs font-bold text-slate-500">
            <span>* 120pie & coffee 공간 조형 및 비주얼 가이드라인 기준</span>
            <span>Slide 06 / 16</span>
          </div>
        </section>
        ...
        <section className={`rounded-3xl p-6 sm:p-12 ${cardBg} relative`}>
          ...
          <div className="space-y-10">
            <div className="text-center md:text-left space-y-3">
              ...
              <p className={`text-sm sm:text-base leading-relaxed ${textDesc}`}>
                기존 매장의 인프라와 120pie의 제품력이 결합되어 극적인 매출 �          <div className="space-y-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-left">
              {[
                ...
                <div key={idx} className={`p-6 rounded-2xl ${innerCardBgAccent} ${innerCardHover} border ${isPink ? "border-neutral-850" : "border-amber-250/20"} flex flex-col justify-between h-[450px] group`}>
                  <div>
        ...
        <section className={`rounded-3xl p-6 sm:p-12 ${cardBg} relative`}>
          ...
          <div className="space-y-10">
            <div className="text-center md:text-left space-y-3">
              <h2 className="text-2xl sm:text-3xl font-black">
                체계화된 <span className={textHighlight}>가맹 메뉴 포트폴리오</span>
              </h2>
              <p className={`text-sm sm:text-base leading-relaxed ${textDesc}`}>
                상권 선호도에 맞춰 선택 도입하고 결합 판매하기 매우 용이한 구성입니다.
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center text-left">
              <div className="lg:col-span-8 grid grid-cols-1 sm:grid-cols-2 gap-4">
                {[
                  { cat: "🥐 120겹 파이 시리즈", desc: "시그니처 바삭 패스츄리 파이", items: ["애플파이", "블루베리파이", "불고기파이", "불닭파이", "크림치즈파이"], icon: <Building2 size={16} className="text-amber-450" /> },
                  { cat: "🥚 에그120 계란빵", desc: "쌀가루 기반 프리미엄 델리", items: ["오리지널에그빵", "베이컨에그빵", "콘버터에그빵", "슈크림에그빵"], icon: <Sparkles size={16} className="text-amber-450" /> },
                  { cat: "🍡 츄러스120 시리즈", desc: "바삭쫄깃 스패니시 냉동 츄러스", items: ["시나몬츄러스", "오레오츄러스", "밀크초코츄러스", "크림치즈츄러스"], icon: <Leaf size={16} className="text-amber-450" /> },
                  { cat: "☕ 커피 & 드링크", desc: "최고 궁합 원두 및 에이드", items: ["하우스 아라비카", "헤이즐넛 라떼", "특제 자몽 에이드", "밀크 버블티"], icon: <Coffee size={16} className="text-amber-450" /> }
                ].map((cat, idx) => (
                  <div key={idx} className={`p-5 rounded-2xl ${innerCardBgAccent} ${innerCardHover} flex flex-col justify-between h-[210px]`}>
                    <div>
                      <div className={`flex justify-between items-start border-b ${isPink ? "border-neutral-850" : "border-amber-200/30"} pb-2 mb-3`}>
                        <h4 className={`text-xs sm:text-sm font-black ${textTitle}`}>{cat.cat}</h4>
                        {cat.icon}
                      </div>
                      <span className={`text-[10px] ${textDesc} font-bold block mb-3 leading-none`}>{cat.desc}</span>
                      <div className={`space-y-1 text-xs ${textDesc} font-semibold grid grid-cols-2 gap-x-2`}>
                        {cat.items.map((it, itemIdx) => (
                          <div key={itemIdx} className="flex items-center gap-1.5 py-0.5">
                            <span className={`w-1.5 h-1.5 ${isPink ? "bg-rose-500" : "bg-amber-400"} rounded-full shrink-0`}></span>
                            <span>{it}</span>
                          </div>
                        ))}
              ...
              <div className="lg:col-span-4 relative group">
                <div className="relative rounded-2xl overflow-hidden border border-neutral-200/20 shadow-xl aspect-[4/5] bg-neutral-950">
                  <img 
                    src="https://res.cloudinary.com/dx7l09wwu/image/upload/v1779945185/120%EA%B2%B9%ED%8C%8C%EC%9D%B4_%EC%97%B0%EC%B6%951_ueicna.jpg" 
                    alt="120pie premium products" 
                    className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500 opacity-90"
                  />
                  <div className="absolute inset-0 bg-gradient-to-t from-neutral-950/70 via-transparent to-transparent flex items-end p-4">
                    <span className="text-xs font-bold text-white/95">오븐에서 갓 베이킹된 120겹 파이 플레이터</span>
                  </div>
                </div>
              </div>
            </div>
        ...
        <section className={`rounded-3xl p-6 sm:p-12 ${cardBg} relative`}>
          ...
          <div className="space-y-10">
            <div className="text-center md:text-left space-y-3">
              <h2 className="text-2xl sm:text-3xl font-black">
                고객 재방문을 부르는 <span className={textHighlight}>세부 시그니처 메뉴</span>
              </h2>
              <p className={`text-sm sm:text-base leading-relaxed ${textDesc}`}>
                신선한 필링과 특제 소스로 속을 꽉 채워 한 입 베어물면 잊지 못할 풍미를 선사합니다.
              </p>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {[
                { name: "오리지널 애플파이", type: "SWEET DESSERT", rating: "★★★★★ (4.9)", desc: "사과 과육이 가득 씹히는 달콤한 시나몬 애플 필링이 특징인 부동의 1위 메뉴", img: "https://res.cloudinary.com/dx7l09wwu/image/upload/v1779945185/120%EA%B2%B9%ED%8C%8C%EC%9D%B4_%EC%97%B0%EC%B6%9C1_ueicna.jpg" },
                { name: "직화 불고기파이", type: "SAVORY MEAL", rating: "★★★★★ (4.8)", desc: "달콤짭조름한 숯불구이 향의 소고기 토핑이 가득 들어간 든든한 식사 대용 파이", img: "https://res.cloudinary.com/dx7l09wwu/image/upload/v1779945185/120%EA%B2%B9%ED%8C%8C%EC%9D%B4_%EC%97%B0%EC%B6%9C4_r90tky.jpg" },
                { name: "동물복지 에그120", type: "DELI EGG BREAD", rating: "★★★★★ (4.9)", desc: "국산 쌀가루 반죽에 친환경 유황 유정란을 통째로 얹은 프리미엄 폭신한 계란빵", img: "https://res.cloudinary.com/dx7l09wwu/image/upload/v1779945603/A4_08297_2_kkxovy.jpg" },
                { name: "오리지널 시나몬츄러스", type: "SPANISH CHURROS", rating: "★★★★★ (4.7)", desc: "주문 즉시 튀기거나 구워 시나몬 슈가를 골고루 묻혀낸 바삭하고 쫄깃한 간식", img: "https://res.cloudinary.com/dx7l09wwu/image/upload/v1779759362/IMG_0015_6_3_au1ykg.jpg" }
              ].map((menu, idx) => (
